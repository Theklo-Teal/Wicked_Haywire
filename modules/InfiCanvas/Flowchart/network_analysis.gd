...
class Crawler:
	...
	var root : Dictionary[NetVert, NetVert]  ## The node in [code]orig[/code] that originated a path towards finding any other given node. Origin nodes point to themselves.
	var history : Dictionary[NetVert, NetVert]  ## Nodes from which a node in the keys was found from.
	var visited : Dictionary[NetVert, Array]  ## Tracking connections already checked. Given a node as key, it returns nodes that which connections to we've explored.
	var all_visited : Array[NetVert]  ## Nodes which all acceptable connections have been visited.
	...
	func _init(origins:Array[NetVert]) -> void:
		orig.assign(origins)
	...
	func _conn_accept(curr:NetVert, next:NetVert, node_accept:=Callable()) -> bool:
	...
	func depth_traverse(node_accept:=Callable(), node_find:=Callable()) -> Array[NetVert]:
		iter_finds.clear()
	...
	func breadth_traverse(node_accept:=Callable(), node_find:=Callable()) -> Array[NetVert]:
		iter_finds.clear()
...
## Finds the closest graph target to a given node and sets its [code]NetVert.endnode[/code].
func seek_endnode(from:NetVert) -> Joint:
	if from is Joint:
		from.endnode = null
		return from
	var crawl = Crawler.new([from])
	while crawl.breadth_traverse(Callable(), func(_curr,next): return next is Joint).is_empty() and not crawl.is_finished():
		continue
	from.endnode = crawl.finds.back()
	return from.endnode

## Traverse the whole graphs connected to [code]origins[/code], setting
## [code]NetVert.distan[/code] and [code]NetVert.endnode[/code] of nodes found
## along the way.[br]
## Returns all the Crawlers used to map the network that can be fed into
## [code]find_graphs()[/code] to discriminate distinct graphs of isolated nodes.
func build_flow_field(...origins) -> Array[Crawler]:
	var timestamp = Time.get_ticks_usec()
	var crawls : Array[Crawler]
	var add_crawls : Array[NetVert]
	for node in origins:
		if node is Joint:
	...
	while not finished:
		if not add_crawls.is_empty():
			crawls.append(Crawler.new(add_crawls))
			add_crawls.clear()
		for craw in crawls:
			if craw.is_finished():
				finished = true
...
## [code]NetVert.distan[/code] and [code]NetVert.endnode[/code], assuming the
...
func update_flow_field(paths:Array[Array]):
	...
	while not paths.is_empty():
		...
		for each in paths.size():
			var node : NetVert = paths[p].pop_back()
			node.timestamp = timestamp
			if node is Joint:
				node.endnode = null
				node.distan = 0
...
## Follows the flow field to find a node that is [code]Joint[/code], then returns 
## the path of nodes found along the way.[br]
## If an Joint can't be found, it returns a partial path.
## NOTE, this search can explore out of [code]from[/code] if its [code]NetVert.distan[/code]
## is negative, but won't continue past further nodes of negative value.
func flow_search(from:NetVert) -> Array[NetVert]:
	var rule = Callable(func(curr, next): return (curr.distan == -1 or curr.distan > next.distan) and next.distan >= 0)
	var crawl := Crawler.new([from])
	while crawl.breadth_traverse(rule, func(_curr, next):return next is Joint).is_empty() and not crawl.is_finished():
		continue
